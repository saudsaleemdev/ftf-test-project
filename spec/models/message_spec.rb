...
RSpec.describe Message, type: :model do
  describe "validations" do
    it "validates the maximum length of original_content" do
      message = Message.new(original_content: "a" * (Message::MAX_CONTENT_LENGTH + 1))
      expect(message).to be_invalid
      expect(message.errors[:original_content]).to include("is too long (maximum is #{Message::MAX_CONTENT_LENGTH} characters)")
    end
  ...
  describe "before_save callback" do
    context "when original_content length exceeds MAX_CONTENT_LENGTH" do
    ...
    context "when original_content length is within MAX_CONTENT_LENGTH" do
